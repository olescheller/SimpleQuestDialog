...
local function getActiveQuestInfo(index)
    ...
    return info[index]["title"], info[index]["isComplete"], info[index]["isLegendary"],
        info[index]["frequency"], info[index]["isRepeatable"], info[index]["isCampaign"], info[index]["isConvenantCalling"] 
end
    ...
    local scale,x,y=UIParent:GetEffectiveScale(),GetCursorPosition();
      ...
      elseif isOverGossip then
        if C_GossipInfo.GetNumActiveQuests() > 0 then
            ...
            t:SetText("Click to select " .. "|T" .. QuestUtil.GetQuestIconActive( isComplete, isLegendary, frequency, isRepeatable, isCampaign, isCovenantCalling) .. ":16:16:0:0:64:64:4:60:4:60|t " .. activeQuestName .. "")
            resizeTooltip()
            f:Show()
            f:SetPoint("BOTTOMLEFT", UIParent, x/scale+10, y/scale)
        else if
            C_GossipInfo.GetNumAvailableQuests() > 0 then
                local availableQuestName, isLegendary, frequency, isRepeatable, isCampaign, isCovenantCalling = getAvailableQuestInfo(1)
                t:SetText("Click to select " .. "|T" .. QuestUtil.GetQuestIconOffer( isLegendary, frequency, isRepeatable, isCampaign, isCovenantCalling) .. ":16:16:0:0:64:64:4:60:4:60|t " .. availableQuestName .. "")
                resizeTooltip()
                f:Show()
                f:SetPoint("BOTTOMLEFT", UIParent, x/scale+10, y/scale)
            end
